<template>
  <div class="Main">
    <div>
      ...
      <el-dialog v-model="dialogFormVisible" title="新增分类" width="500px" @closed="resetForm">
        <el-form :model="form" style="margin-right: 120px" ref="ruleFormRef" :rules="rules">
          ...
          <el-form-item required label="分类名称" :label-width="formLabelWidth" prop="name">
    ...
    <el-table :data="goodsTypeData.list" style="width: 100%" border="border">
...
export default {
  ...
  setup() {
    ...
    const search=async () => {
      const {data} = await post(httpUrl+"/goodsType/listPage", {
        pageSize: pageSize,
    ...
    const handleDelete = async (row) => {
      ElMessageBox.confirm(
        ...
        const {code} = await get(httpUrl+"/goodsType/delete", {id:row.id})
        if (code === 200) {
          ElMessage({
            message: '删除成功',
            type: 'success',
    ...
    const submit=async (formEl) => {
      ...
      formEl.validate(async (valid) => {
        if (valid) {
          ...
          console.log(form.id)
          const {code} = await post(httpUrl+"/goodsType/saveOrMod", form)
          if (code === 200) {
            ElMessage({
              message: '操作成功',
              type: 'success',
